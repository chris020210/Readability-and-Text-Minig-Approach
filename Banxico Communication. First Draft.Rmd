...
The changes originated, both in theory and practice, over the past 30 years, have enabled that monetary authorities tend to communicate more and share a wider range of information with the public. However, it should be noted that transparency does not always imply communication, nor does it always imply that the information that is shared is clear or easily understood.

Given the great power that central banks have to potentially shape expectations and affect the economy, it is not only important to increase the amount of information shared, but to ensure that it is clear and intelligible, provides relevant insights, encourages two-way channels and allows for public scrutiny. In addition, care must be taken to ensure that the information shared can be understood by the widest possible audience, not just highly specialized experts and professional observers who can easily interpret the central bank's message.
...
knitr::kable(FLESCH.SCALE,
  ...
  column_spec(3, background = color_F,
              color = "black", width = "3cm",bold = T) %>%
  add_footnote("Source: Fernández Huerta(1959), Szigriszt-Pazos(1992), Muñoz (2006) and Cantalejo et al. (2008)",
               notation = "none")
```



Table 4 displays the basic descriptive statistics of the word and sentence count. On average, MPSs have the shortest extension, 1,101 and 37 words and sentences respectively, and QIRs the largest extension, 17,546 and 548. The MPMs find in between with and average count of 6,148 words and 178 sentences.
...
A<- Banxico_readability %>%
  mutate(words = as.numeric(words),
         sentences = as.numeric(sentences))%>%
  select("Year", "document", "words", "sentences") %>%
  pivot_longer(cols = c(sentences,words),
               names_to = "Count", 
               values_to = "Count_vals") %>%
  group_by(document,Count) %>%
  summarise(n = length(document),
            mean = mean(Count_vals),
            median = median(Count_vals),
            sd = sd(Count_vals),
            min = min(Count_vals),
            max = max(Count_vals))
...
Figure1 %>%
  ...
  labs( 
    title = "Figure 1: Banco de México's publications - Word count",
...
A$type <- c("-","Dissemination and Specialized press","-","Literature and Dissemination",
            
            "-","Dissemination and Specialized press","-","Literature and Dissemination",
...
knitr::kable(A,
  ...
  col.names = c("Test","Document","Average" ,"Scale","Grade Level", "Type")) %>%
    ...
    column_spec(c(3,4), background = color,
              color = "black",bold = T) %>%
    add_footnote(c("Source: Author's calculation."),
                 notation = "none",
                 threeparttable = F)
...
CENSUS_EDUC_LEV %>% 
    ...
    labs(
    title = "Figure 3: Population over 15 years old by educational level",
    x = "",
    y = "",
    color = "",
    fill = "",
    caption = "Source: Author's calculation.
Note: Basic = Elementary and Junior high; Upper middle = High School; Superior = University and Graduate.")+
  theme_tufte(base_family = "serif",
            base_size = 12)+
  scale_fill_wsj()+
...
\noindent
```{r Figure 3  Scatter Plots, echo=FALSE, fig.height=4.5, fig.width=6, message=FALSE, warning=FALSE}
...
multiplot(COR_MPM,COR_MPS,COR_QIR,cols = 3)
