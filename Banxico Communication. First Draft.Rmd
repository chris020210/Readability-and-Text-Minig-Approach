...
This paper aims to fill the gap existing in literature representing a primer contribution to analyze the readability and perspicuity of Banco de México's official documents, as well as the implementation of text analysis techniques to scrutinize the wording and writing style of the Board of Directors over time. I address the question of whether there have been significant changes in central bank communication, especially following the implementation of an Inflation Forecast Targeting strategy (IFT) in 2018 and the update of the Board of Directors communication criteria in 2020. To do so I perform classic readability and perspicuity measures on the monetary policy statements, the minutes, and the quarterly inflation reports. Then I applied unit root tests that allow for one and two endogenous structural breaks. [insert relevant conclusion of the unit root analysis]. Then, I move forward by analyzing the wording of the minutes and the monetary policy statements [insert findings of text analysis].

The rest of the paper is organized as follows. Section 2 summarizes the literature on central bank communication. Section 3 describes the methodology and the data. Section 4 presents the results of the readability and structural break analysis. Section 5 shows the changes in Banco de México's wording through the presentation of high-frequency words and clustering techniques. Finally, Section 6 concludes.
...
The work of Christina and David Romer (1989)[^9] represents a seminal contribution to communication literature. Applying the so-called _narrative approach_, they investigate a set of periods in which the Federal Reserve's wording was restrictive to study its relationship to the real effects of monetary shocks. They find that narratives, i.e., the historical record, such as descriptions of the process and reasoning that led to the monetary authority's decisions and accounts of the sources of monetary disturbances, can solve the problem of identifying multiple macroeconomic issues.
...
The growing power of monetary authorities and the importance of market expectations in the transmission of monetary policy has yielded new definitions of monetary policy strategy and the central banker toolkit. In the aftermath of the GFC, the central bank's role has been characterized by an increase in procedural transparency and an evolution of clarity in communication. With the development of communication tools, a lot of literature has emerged trying to understand the impact of central bank's communication on market expectations, signals on the yield curve, and the effective transmission of monetary policy.

During the 2010s, several papers focused on communication and its application under the ZLB in developed countries appeared[^19]. For exmaple, Campbell et al. (2012) and Campbell (2013), using both an empirical an theoretical approach, showed how the Fed, through public announcements and forward guidance, can successfully influence assets prices and forecast over a variety of many macroeconomics variables. Their work also derived in new definitions of communication and the types of forward guidance a monetary authority can deliver.[^20]
...
color_INFLESZ = c(rep("#e3342f",5),rep("#f6993f",3),rep("#00b159",2),
                  rep("#4dc0b5",3),rep("#3990dc",4))
color_INFLESZ = rev(color_INFLESZ)

color_FSZ = c(rep("#e3342f",2),rep("#f6993f",2),rep("#ffed4a",3),
              rep("#00b159",3),rep("#4dc0b5",2),rep("#3990dc",2),
              rep("#6574cd",3))
color_FSZ = rev(color_FSZ)

color_F = c(rep("#e3342f",3),rep("#f6993f",4),rep("#ffed4a",2),
              rep("#00b159",2),rep("#4dc0b5",2),rep("#3990dc",2),
              rep("#6574cd",2))
color_F = rev(color_F)
