...
One of the reasons for being very cautious with traditional readability measures is that they punish with lower punctuation the more complex the words and the longer the sentences. However, in fields such as economics, there are many complex words that are easy to understand, and in many cases, longer sentences can provide a clear message about the information the central bank wants to convey.[^28] Joan Huang and John Simon also emphasize that better readability does not mean the central bank is successfully explaining its actions and the motivation behind them.[^29] The reason is that while a comprehensive explanation may not be simple or easily understood, a simple explanation may not be accurate.


[^28]: [See Janan and Wray (2013)](https://hipatiapress.com/hpjournals/index.php/remie/article/view/remie.2012.04).

[^29]: [Joan Huang and John Simon (2021)](https://apo.org.au/node/312370).    
...
Then, is arguably that LP test is not the most appropriate for our data. Instead, Lee and Strazicich (2003), LS test is suitable because expands the analysis to allow for two endogenous unknown breakpoints both under the null and alternative hypothesis. Hence, the model specifications of Ls test is a unit root process with up to two structural breaks in level, trend or both, against the alternative hypothesis of an stationary process with up to two breaks in level, trend or both. In addition, the bootstrap approach by [Chou (2007)](https://www.sciencedirect.com/science/article/pii/S0165176506002722) is implemented to derive the critical values of the t-statics and the distributional properties of the t-statics and the selected break points.[^31]

[^31]: For a similar implementation of this procedure see [Lips (2017)](https://www.degruyter.com/document/doi/10.1515/jtse-2016-0018/html).
...
knitr::kable(Onebreak_Unit,
             caption = "Unit Root tests with two endogenous structural break",
             align = "c",
             booktabs = T,
    ...
    column_spec(4,  width = "3cm") %>%
      #column_spec(4, background = color,
       #       color = "black",bold = T)  %>%
    add_header_above(c(" " = 4, "LS CV" = 3, "Bootstrap CV" = 3),
                     bold = T, font_size = 11)%>%
    add_footnote(c("Source: Author's calcultion.","Note: rejection of the null at (*)1%, (**)5%, (***)10% level.", "The date in brackets represent the potentila point break."),
                 notation = "none",
                 threeparttable = F)
...
In the search for the adaptation of the readability formulas to the texts written in Spanish, José Fernández Huerta developed a first measure to test the reading ease of a wide variety of Spanish documents in 1959.[^] The Fernández Huerta test is based on the famous Flesch test and they have similar readability scale. The formula is as follows:
...
```{r Table Flesch-Fernández Huerta Score, echo=FALSE}
...
knitr::kable(FLESCH.SCALE,
             caption = "Fernández Huerta Readability Scale",
             align = "c",
             booktabs = T)  %>%
  kable_styling(latex_options = c("hold_position"),
                full_width = F) %>%
...
\begin{equation}
  F_{SZ} = 207 - \frac{W}{St} -62.3 * \frac{Sy}{W}
\end{equation}
...
In a later work, Barrio-Cantalejo adapted the Szigriszt-Pazos scale of interpretation of results, creating the INFLESZ scale and a new validated perspicuity formula, the Flesch-Szigritsz.[^]
...
$FSZ = 206.835 -62.3 (\frac{Sy}{W} - \frac{W}{S})$
...
FLESCH.SCALE <- data.frame("Score" = c("80-100",
                           ...
                           "Type" = c("Comics", 
                                      ...
                                      "Scientific dissemination and Quality magazines",
                                     "Scientific"))
