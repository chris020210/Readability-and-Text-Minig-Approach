...
For central banks, communication is not only desirable but necessary. This follows the rationale that, on the one hand, central bank communication owns the power to shape expectations of future macroeconomic outcomes, thus improving the transmission of monetary policy, and on the other, of the need for greater transparency since central bankers are not elected by democratic suffrage, but are appointed by the president in office, so that, in a democratic society, transparency allows their actions to be judged by the public they serve through accountability mechanisms.

The latter has been extensively studied by the literature on transparency[^1], leading to transparency indicators and the consensus that central banks should share information on their objectives, the way they evaluate the economy as well as the decisions and monetary policy decision-making process. Although, it must be said that transparency does not always imply communication, nor that the central bank is conveying a clear message.
...
This paper aims to fill the existing gap in the literature by representing a first contribution to analyze the readability and perspicuity of Banco de México's official documents over time. I address the question of whether there have been significant changes in the central bank's communication, especially after the implementation of an Inflation Forecast Targeting (IFT) strategy in 2018 and the update of the Board of Governors' communication criteria in 2020. To do so, I review the main changes to the communication strategy in recent years. I then compute classical measures of readability and perspicuity on monetary policy statements, minutes and quarterly inflation reports, and argue that these measures can be considered as a proxy for the clarity and intelligibility of Banco de México's wording. Finally, I move forward by performing unit root tests that allow for one and two endogenous structural breaks to confirm whether there are significant structural changes in central bank clarity. [insert relevant finding from unit root analysis].

The rest of the paper is organized as follows. Section 2 summarizes the literature on central bank communication. Section 3 present the data and review the evolution of Banco de México's communication strategy. Section 4 presents the results of the linguistic analysis. Afterwards, section 5 shows the findings of the structural change tests. Finally, Section 6 concludes.
...
knitr::kable(Banxico_Communication,
             caption = "Structure of Banxico's official documents.",
             booktabs = T,
             longtable = F,
...
FLESCH.SCALE <- data.frame("Score" = c(100,95,90,85,80,75,70,65,60,55,50,
                           ...
                           "Szigriszt" = c(rep("Very easy",3), rep("Easy", 2),
                                           ...
                                           rep("Fairly difficult",3), rep("Difficult",2), 
                                           rep("Very difficult",2)),
                           "Flesch" = c(rep("Very easy",2), rep("Easy", 2),
                                           rep("Fairly easy",2),rep("Standard",2), 
                                           rep("Fairly difficult",2), rep("Difficult",4), 
                                           rep("Very difficult",3)))
...
knitr::kable(FLESCH.SCALE,
  ...
  column_spec(2, background = color_INFLESZ,
              color = "black", width = "4cm",bold = T) %>%
  column_spec(3, background = color_FSZ,
              color = "black",width = "4cm",bold = T) %>%
  column_spec(4, background = color_F,
              color = "black", width = "4cm",bold = T) %>%
...
Banxico.Readability1122$document[Banxico.Readability1122$document == "IQR"] <- "QIR"
Banxico.Readability1122$document[Banxico.Readability1122$document == "MPD"] <- "MPS"
...
Table 6 displays the total documents per type (N), the average numbers of words (Mean), the median of the distribution of words per document (Median), standard deviation (SD), the shortest document (Min.) and the largest document (Max.) over the period from 2011 to 2022. On average, the Monetary Policy Statements have the shortest extension, 1,223 total words, and the Inflation Quarterly Reports the largest extension, 15,546. 
...
Banxico_readability <- Banxico.Readability1122
