...
[^3]: See [Faust and Svensson (2001)](https://www.jstor.org/stable/2648735).
...
[^6]: See [Kydland and Prescott (1977)](https://www.jstor.org/stable/1830193), [Calvo (1978)](https://www.jstor.org/stable/pdf/1913836.pdf), [Barro and Gordon (1983)](https://www.sciencedirect.com/science/article/pii/030439328390051X), [Rogoff (1985)](https://www.jstor.org/stable/1885679).
...
[^7]: See [Blinder (2004).](https://yalebooks.yale.edu/book/9780300100877/the-quiet-revolution/)

The work of Christina and David Romer (1989)[^8] is a seminal contribution to communication literature. Applying the so-called _narrative approach_, they investigate a set of periods in which the Federal Reserve's verbal stance was restrictive. They find that narratives, i.e., the historical record, the descriptions of the process and reasoning that led to the monetary authority's decisions and accounts of the sources of monetary disturbances, are related to multiple macroeconomic outcomes, such as the real effects of monetary shocks.

[8]: See [Romer and Romer (1989).](https://www.nber.org/system/files/chapters/c10964/c10964.pdf)
...
[^9]: See [Blinder (1999).](http://www.artsrn.ualberta.ca/econweb/landon/1999%20Blinder%20Central%20Banking.pdf)
...
[^10]: See [Woodford (2001).](https://www.kansascityfed.org/Jackson%20Hole/documents/3495/papers-S02WOOD.pdf)
...
[^22]: See King (1994), Svensson (1997; 1999, 2008), Clinton et al. (2015), Svensson (2020)
[^23]: See [Campbell et al. (2012)](https://www.brookings.edu/wp-content/uploads/2012/03/2012a_evans.pdf)
...
A%>%
  ...
  geom_line(size = 0.7)+
    ...
    y = " ",
    color = " ",
    caption = "**Figure 2**: Readability of Banco de México over time.<br><br>Source: Author's calculation.<br>Note: The average was calculated using the standard mean at each time position of the three readability<br>measures. The dashed lines correspond to the respect readability level on the INFLESZ scale. Harry<br>Potter denotes the minimum score of the famous novel *Harry Potter and the Philosopher's Stone*.")+
  theme_tufte(base_family = "serif",
            base_size = 11)+
  #scale_color_brewer(palette="Set1")+
...
CENSUS_EDUC_LEV %>% 
    ...
    color = "",
    fill = "",
    caption = "**Figure 3**: Population over 15 years old by educational level<br><br>Source: INEGI.<br>Note: Basic = Elementary and Junior high; Upper middle = High School;<br>Superior = University and Graduate.")+
  theme_tufte(base_family = "serif",
              base_size = 11)+
  #scale_fill_wsj()+
 ...
 geom_text(aes(label = scales::percent(round(percent,2))), 
           ...
           show.legend = F, size =c(3,5,5,5,5),
           family = "serif",
...
One of the reasons for being very cautious with traditional readability measures is that they punish with lower punctuation the more complex the words and the longer the sentences. However, in fields such as economics, there are many complex words that are easy to understand, and in many cases, longer sentences can provide a clear message about the information the central bank wants to convey.[^27] Joan Huang and John Simon also emphasize that better readability does not mean the central bank is successfully explaining its actions and the motivation behind them.[^28] The reason is that while a comprehensive explanation may not be simple or easily understood, a simple explanation may not be accurate.


[^27]: See Janan and Wray (2012).

[^28]: Joan Huang and John Simon (2021).    
...
COR_MPS <- A %>%
  ...
  mutate(sentences = as.numeric(sentences),
    ...
    subtitle = "
         ...
         y = "",
         x = "FSZ score",
      caption = "**Figure 4**: Scatter plots of linguistic variables.<br><br>Source: Author's calculation.") +
    theme_tufte(base_family = "serif",
            base_size = 11)+
    #scale_color_brewer(palette="Set1") +
    #scale_color_manual(values = vintage)+
...
The evolution of Banco de México's verbal clarity is strongly linked with institution-specific factors. However, it is possible to identify idiosyncratic biased related to the chairman in office[^29]. Figure 4 shows fitted calculations for total perspicuity of Banxico's publications in the tenures of Agustín Carstens (Jan 10-Nov 17), Alejandro Díaz de León (Dec 17- Dec 21), and Victoria Rodríguez (Jan 22-).

[^29]: For exmaple, [Bernanke (2022)](https://wwnorton.com/books/9781324020462) narrates how the Fed's monetary policy decisions have historically been strongly biased by the figure of the chairman, especially referring to the great persuasive power of Paul Volcker and Alan Greenspan over the decisions of the other members of the Federal Open Market Committee (FOMC).
...
TS_data3 <- TS_data %>%
    pivot_longer(cols = c(MPSs.5MA.,MPMs.5MA.),
                 names_to = "MA",
                 values_to = "MA_vals") %>%
...
#TS_data3 <- TS_data3 %>%
 #   mutate("LOCF" = TS_data2$LOCF,
  #         "LOCF_vals" = TS_data2$LOCF_vals) %>%
    #mutate(document = substring(LOCF, first = 1, last = 3))
...
TS_data3%>%
  ...
  labs(title = "",
        ...
        y = " FSZ score",
    color = " ",
      caption = "**Figure 6**: Transformed monthly series.<br><br>Source: Author's calculation.")+
    theme_tufte(base_family = "serif",
            base_size = 11)+
  #scale_color_brewer(palette="Set1")+
  #scale_color_manual(values = winter)+
...
For the clarity levels of the three publications, classic unit root tests such as the Augmented Dicky-Fuller [ADF] (Dicky and Fuller, 1981), Phillips-Perron [PP] (Phillips and Perron, 1988) and KPSS (Kwiatkowski et al., 1992) are executed to validate the hypothesis of sationarity in the Data Generating Process (DGP) of the series. The unit root specification includes models with no drift no trend (type A), drift and no trend (type B), and drift with trend (type C) respectively. Overall, there is no convincing evidence against the unit root null-hypothesis (See Appendix C). However, for each series, the KPPS test gives mix evidence on non-stationarity process.

Since the seminal paper by Perron ([1989](https://www.jstor.org/stable/1913712?searchText=&searchUri=&ab_segments=&searchKey=&refreqid=fastly-default%3A5187955ea05417ef3e6d4821c2f07ea9)), the literature has stressed that in the presence of one or more structural breaks, the non-rejection of the unit root null may be biased. Perron's test is a variation of the ADF test that takes into account the possibility of a known exogenous structural break in the time series. Perron relied on previous observation of the macroeconomic series studied by Nelson and Plosser (1982), arguing then that the latter's conclusion of integrated process might be biased due to the presence of structural changes in the U.S. economy during the years of the "Great Depression" and the 1973 oil price shock.

In subsequente contribution, Zivot and Andrews (ZA)([1992](https://www.jstor.org/stable/1391541?searchText=&searchUri=&ab_segments=&searchKey=&refreqid=fastly-default%3A22b362ee8348ccf8b5607097327edae9)), Banerjee, Lumsdaine, and Stock (BLS)(1992), and and Perron and Vogelsang (PV)([1992](https://www.jstor.org/stable/1391823?searchText=&searchUri=&ab_segments=&searchKey=&refreqid=fastly-default%3A8d8282cdb8423068941d433f745a7557); [1998](https://www.jstor.org/stable/2527353)), reviewed Perron's conclusion and criticized the exogenous structural break identification. They propose tests which endogenized breakpoint selection. Like Perron, these tests allow for the presence of structural break in th level of the series ("crash" model), a change in the trend ("changing growth" model), or both. 

For purpose of this paper ZA and PV test are performed. The method for the selection of the type of break as well as the lags for the augmented form models were selected using both information criteria and general to specific approach for a maximum of 12 lags in the case of the monthly series and 8 in the quarterly series. In every model we found for the minimum information criteria and statistic significance at the 5% for the maximum lag coefficient.
...
```{r Tab 5 URT one breal, echo=FALSE}
...
knitr::kable(Onebreak_Unit,
    ...
    add_header_above(c(" " = 2, "ZA" = 2, "PV" = 2),
                     bold = T)%>%
    add_footnote(c("Source: Author's calculation.","Note: rejection of the null at (*)1%, (**)5%, (***)10% level.", "The date in brackets represent the potentila point break."),
                 notation = "none",
                 threeparttable = F)
...
From an economic perspective, there is no reason to limit the analysis to a single structural change. Moreover, Lumsdaine and Papell (LP)(1997) showed that allowing for only one point break could biased the results and conclusions of the previous tests, therefore, they extended the analysis to allow for two endogenous unknown structural breaks. They also conclude that in test such as the ZA which does not allow for breaks under the unit root null, excludes all the other cases to the alternative hypothesis, i.e., the alternative hypothesis implies that there are breaks in the series including the possibility of unit root with breaks.

Then, is arguably that LP test is not the most appropriate for our data. Instead, Lee and Strazicich (2003), LS test is suitable because expands the analysis to allow for two endogenous unknown breakpoints both under the null and alternative hypothesis. Hence, the model specifications of Ls test is a unit root process with up to two structural breaks in level, trend or both, against the alternative hypothesis of an stationary process with up to two breaks in level, trend or both. In addition, the bootstrap approach by Chou (2007) is implemented to derive the critical values of the t-statics and the distributional properties of the t-statics and the selected break points.[^30]

[^30]: For a similar implementation of this procedure see [Lips (2017)]().
