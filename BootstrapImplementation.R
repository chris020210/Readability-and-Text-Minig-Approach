...
boot.y <- foreach(i=1:n.sim, .combine = rbind, .packages = 'foreach') %dopar%{
  ...
  #plotl(y.star)
  test.stat <- ur.ls.bootstrap(y=y.star , model = model, breaks = breaks , method = "GTOS",pn = 0.1, critval = "bootstrap", print.results = "silent")
  if(breaks == 2){
...
IQR_BOOTS_RESULTS <- test.results.y
IQR_BOOTS_INTERVALS <- boot.y
...
myVariable <- na.omit(as.numeric(MPD_MAFSZ))
...
quantile(unlist(MPD_BOOTS_INTERVALS[,"t-stat"]), c(.01, .05, .1, .25, .5, .75, .95, .99))
quantile(unlist(MPD_BOOTS_INTERVALS[,"First break"]), c(.01, .05, .1, .25, .5, .75, .95))
quantile(unlist(MPD_BOOTS_INTERVALS[,"Second break"]), c(.01, .05, .1, .25, .5, .75, .95))
...
myVariable <- na.omit(as.numeric(MPM_MAFSZ))
...
test.results.y <-  ur.ls.bootstrap(y=myVariable , model = model, breaks = breaks, lags = 5, method = "Fixed",pn = 0.1, critval = "bootstrap", print.results = "print")
...
boot.y <- foreach(i=1:n.sim, .combine = rbind, .packages = 'foreach') %dopar%{
  ...
  #y.star.matrix[,i] <- y.star
  #plotl(y.star)
  test.stat <- ur.ls.bootstrap(y=y.star , model = model, breaks = breaks, lags = 5, method = "Fixed",pn = 0.1, critval = "bootstrap", print.results = "silent")
  if(breaks == 2){
    return(list(c(unlist(test.stat[[1]]$`t-stat`)), c(unlist(test.stat[[1]]$`First break`)) ,c(unlist(test.stat[[1]]$`Second break`)),  c(as.numeric(test.stat[[1]]$Runtime, units = "mins"))))
  } else if (breaks == 1){
    return(c(c(unlist(test.stat[[1]]$`t-stat`)), c(unlist(test.stat[[1]]$`First break`)) , c(as.numeric(test.stat[[1]]$Runtime, units = "mins"))))
...
MPM_BOOTS_RESULTS <- test.results.y
MPM_BOOTS_INTERVALS <- boot.y
