...
CV <- CV %>%
    mutate(density_MPM = dnorm(mean(MPM), sd(MPM)),
           density_MPS = dnorm(mean(MPS), sd(MPS)),
           density_QIR = dnorm(mean(QIR), sd(QIR)))
CV <- CV%>%
    pivot_longer(cols = 1:3, 
                 values_to = "vals",
                 names_to = "names") %>%
...
CV %>%
  ...
  theme_tufte(base_family = "serif", # type of theme
            base_size = 12)+
    scale_colour_economist()+ # color palette
