...
MPD <- Banxico_readability %>%
  filter(document == "MPD") %>% # select MPDs
  select(dates,document,`Flesch-Szigriszt`) %>% # filter interest variables
  # dates to monthly frequency
  mutate(dates = as.Date(format(dates, format= "%Y-%m-01")))
...
IQR <- Banxico_readability %>%
  filter(document == "IQR") %>% # select IQRs
  select(dates,document,`Flesch-Szigriszt`) # filter interest variables
...
```{r VARIABLES TO TS, message=FALSE, warning=FALSE, include=FALSE}

#Minutes
MPM_FSZ <- ts(MPM2$FSZadjusted, start = c(2011,1),
              frequency = 12)
MPM_MAFSZ <- ts(MPM2$MAFSZ, start = c(2011,3),
                frequency = 12) 

#MP Statements
MPD_FSZ <- ts(MPD2$FSZadjusted, start = c(2011,1),
              frequency = 12)
MPD_MAFSZ <- ts(MPD2$MAFSZ, start = c(2011,3),
                frequency = 12)

# Inflation Reports
IQR_FSZ <- ts(IQR$`Flesch-Szigriszt`, start = c(2011,1),
              frequency = 4)
```
...
# For 1 to 6 lags, run each trend model and save the AIC and BIC information criteria. Then, print which one has the minimum.
