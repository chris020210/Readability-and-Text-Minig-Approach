...
library(ggthemes)
library(openxlsx)
write.xlsx(Banxico_readability[,-3], "Linguistic.xlsx")
Banxico_readability %>%
    select(dates,document,TTR,Year) %>%
    mutate(TTR = as.numeric(TTR)) %>%
    group_by(document, Year) %>%
    summarise(mean = mean(TTR)) %>%
    filter(Year %in% c(2011,2022)) %>%
    arrange(desc(Year)) %>%
  ggplot(aes(x= Year, # aesthetics
             y= mean,
             fill = document)) +
  geom_col(aes(x= Year, # line plot
                y= mean), width = 0.5,  
           just = 0.5, 
           position = "dodge", 
           color = "white")+
  ...
  theme(text = element_text(family = "serif") ,
        ...
        axis.line = element_line(linewidth = 0.1, colour = "gray47"),
        axis.ticks = element_line(linewidth = 0.1, colour = "gray47"),
        plot.background = element_blank(),
        panel.background = element_blank(),
        strip.background = element_rect(fill = "aliceblue"),
        axis.text = element_markdown(size = 10, family = "serif", colour = "black"),
...
#Minutes
MPM_FSZ <- ts(MPM2$FSZadjusted, start = c(2011,01),
              frequency = 12)
MPM_MAFSZ <- ts(log(MPM2$MAFSZ), start = c(2011,01),
                frequency = 12)
plot(MPM_MAFSZ)

#MP Statements
MPD_FSZ <- ts(MPD2$FSZadjusted, start = c(2011,01),
              frequency = 12)
MPD_MAFSZ <- ts(log(MPD2$MAFSZ), start = c(2011,01),
                frequency = 12)
plot(MPD_MAFSZ)

# Inflation Reports
IQR_FSZ <- ts(log(IQR$`Flesch-Szigriszt`), start = c(2011,01),
              frequency = 4)
plot(IQR_FSZ)
...
#write.xlsx(IQR_FSZ,"IQRdata.xlsx")
```


# Chech the levels above the fd to read
```{r}
MPM2 %>%
  filter(MAFSZ >= 55)
...
AIC <- c(rep(NA,12)) # AIC vector
BIC <- c(rep(NA,12)) # BIC vector
...
```{r message=FALSE, warning=FALSE}
MPD111 <- ts(MPD_MAFSZ[1:111], start = c(2011,01),
             frequency = 12)
MPD113 <- ts(MPD_MAFSZ[1:113], start = c(2011,01),
             frequency = 12)
...
# zivot andrews test for trend models
AIC <- c(rep(NA,9)) # AIC vector
BIC <- c(rep(NA,9)) # BIC vector
...
```{r message=FALSE, warning=FALSE}
IQR30 <- ts(IQR_FSZ[1:30], start = c(2011,01),
             frequency = 4)

IQR31 <- ts(IQR_FSZ[1:31], start = c(2011,01),
             frequency = 4)

autoplot.zoo(IQR30) +
  labs(title = "Flesch-Szigritz score",
       subtitle = "Before 2018Q4") +
  geom_hline(yintercept = mean(na.omit(IQR30)))

autoplot.zoo(IQR31) +
  labs(title = "Flesch-Szigritz score",
       subtitle = "Before 2018Q4") +
  geom_hline(yintercept = mean(na.omit(IQR31)))
```
...
quantile(unlist(MPM_BOOTS_INTERVALS[,"t-stat"]), c(.01, 0.025, .05, .1, .25, .5, .75, .95, 0.975 ,.99))

quantile(unlist(MPM_BOOTS_INTERVALS[,"First break"]), c(.01, 0.025, .05, .1, .25, .5, .75, .95, 0.975 ,.99))
MPM2$dates[c(17,62)]

quantile(unlist(MPM_BOOTS_INTERVALS[,"Second break"]), c(.01, 0.025, .05, .1, .25, .5, .75, .95, 0.975 ,.99))
MPM2$dates[c(48,121)]
```

```{r}
MPM_BOOTS_INTERVALS <- as.data.frame(MPM_BOOTS_INTERVALS)
MPM_BOOTS_INTERVALS <- MPM_BOOTS_INTERVALS %>%
    mutate(`t-stat` = as.numeric(`t-stat`), 
           `First break` = as.numeric(`First break`),
           `Second break` = as.numeric(`Second break`))
MPM_BOOTS_INTERVALS %>%
    ggplot(aes(x = `t-stat`)) +
    geom_histogram(aes(y = ..density..),
                 colour = 1, fill = "firebrick3",
                 bins =30,
                 position = "identity",
                 binwidth = 0.1, width = .5,alpha = 0.5) +
...
MPM_BOOTS_INTERVALS %>%
    ggplot(aes(x = `Second break`)) +
    geom_histogram(aes(y = ..density..),
                 colour = 1, fill = "firebrick3",
                 binwidth = 3,alpha = 0.5) +
  geom_density(lwd = 1.2,
               linetype = 1,
               colour = "black",
               fill = "firebrick1",
...
```{r}
#Calculate the critical values, based on the distribution of the test statistic
hist(unlist(MPM_BOOTS_INTERVALS_crash[,"t-stat"]), freq = FALSE)
quantile(unlist(MPM_BOOTS_INTERVALS_crash[,"t-stat"]), c(.01, 0.025, .05, .1, .25, .5, .75, .95, 0.9725 ,.99))
quantile(unlist(MPM_BOOTS_INTERVALS_crash[,"First break"]), c(.01, 0.025, .05, .1, .25, .5, .75, .95, 0.9725 ,.99))
quantile(unlist(MPM_BOOTS_INTERVALS_crash[,"Second break"]), c(.01, 0.025, .05, .1, .25, .5, .75, .95, 0.9725 ,.99))
```


```{r}
MPM_BOOTS_INTERVALS_crash <- as.data.frame(MPM_BOOTS_INTERVALS_crash)
MPM_BOOTS_INTERVALS_crash <- MPM_BOOTS_INTERVALS_crash %>%
    mutate(`t-stat` = as.numeric(`t-stat`), 
           `First break` = as.numeric(`First break`),
           `Second break` = as.numeric(`Second break`))
MPM_BOOTS_INTERVALS_crash %>%
    ggplot(aes(x = `t-stat`)) +
    geom_histogram(aes(y = ..density..),
                 colour = 1, fill = "firebrick3",
                 bins =30,
                 position = "identity",
                 binwidth = 0.1, width = .5,
                 alpha = 0.25) +
  geom_density(lwd = 1.2,
               linetype = 1,
               colour = "black",
...
quantile(unlist(MPD_BOOTS_INTERVALS[,"t-stat"]), c(.01, 0.025, .05, .1, .25, .5, .75, .95, 0.975 ,.99))

quantile(unlist(MPD_BOOTS_INTERVALS[,"First break"]), c(.01, 0.025, .05, .1, .25, .5, .75, .95, 0.975 ,.99))
MPD2$dates[c(16,93)]

quantile(unlist(MPD_BOOTS_INTERVALS[,"Second break"]), c(.01, 0.025, .05, .1, .25, .5, .75, .95, 0.975 ,.99))
MPD2$dates[c(69,123)]

```

```{r}
MPD_BOOTS_INTERVALS <- as.data.frame(MPD_BOOTS_INTERVALS)
MPD_BOOTS_INTERVALS <- MPD_BOOTS_INTERVALS %>%
    mutate(`t-stat` = as.numeric(`t-stat`), 
           `First break` = as.numeric(`First break`),
           `Second break` = as.numeric(`Second break`))
MPD_BOOTS_INTERVALS %>%
...
```{r}
#Calculate the critical values, based on the distribution of the test statistic
hist(unlist(MPD_BOOTS_INTERVALS_crash[,"t-stat"]), freq = FALSE)
quantile(unlist(MPD_BOOTS_INTERVALS_crash[,"t-stat"]), c(.01, 0.025, .05, .1, .25, .5, .75, .95, 0.9725 ,.99))
quantile(unlist(MPD_BOOTS_INTERVALS_crash[,"First break"]), c(.01, 0.025, .05, .1, .25, .5, .75, .95, 0.9725 ,.99))
quantile(unlist(MPD_BOOTS_INTERVALS_crash[,"Second break"]), c(.01, 0.025, .05, .1, .25, .5, .75, .95, 0.9725 ,.99))
```


```{r}
MPD_BOOTS_INTERVALS_crash <- as.data.frame(MPD_BOOTS_INTERVALS_crash)
MPD_BOOTS_INTERVALS_crash <- MPD_BOOTS_INTERVALS_crash %>%
    mutate(`t-stat` = as.numeric(`t-stat`), 
           `First break` = as.numeric(`First break`),
           `Second break` = as.numeric(`Second break`))
MPD_BOOTS_INTERVALS_crash %>%
    ggplot(aes(x = `t-stat`)) +
    geom_histogram(aes(y = ..density..),
                 colour = 1, fill = "firebrick3",
                 bins =30,
...
#Calculate the critical values, based on the distribution of the test statistic
hist(unlist(IQR_BOOTS_INTERVALS[,"t-stat"]), freq = FALSE)

quantile(unlist(IQR_BOOTS_INTERVALS[,"t-stat"]), c(.01, 0.025, .05, .1, .25, .5, .75, .95, 0.975 ,.99))

quantile(unlist(IQR_BOOTS_INTERVALS[,"First break"]), c(.01, 0.025, .05, .1, .25, .5, .75, .95, 0.975 ,.99))
IQR$Q[c(11,33)]

quantile(unlist(IQR_BOOTS_INTERVALS[,"Second break"]), c(.01, 0.025, .05, .1, .25, .5, .75, .95, 0.975 ,.99))
IQR$Q[c(24,42)]

```

```{r}
IQR_BOOTS_INTERVALS <- as.data.frame(IQR_BOOTS_INTERVALS)
IQR_BOOTS_INTERVALS <- IQR_BOOTS_INTERVALS %>%
    mutate(`t-stat` = as.numeric(`t-stat`), 
           `First break` = as.numeric(`First break`),
           `Second break` = as.numeric(`Second break`))
IQR_BOOTS_INTERVALS %>%
...
```{r}
#Calculate the critical values, based on the distribution of the test statistic
hist(unlist(IQR_BOOTS_INTERVALS_crash[,"t-stat"]), freq = FALSE)
quantile(unlist(IQR_BOOTS_INTERVALS_crash[,"t-stat"]), c(.01, 0.025, .05, .1, .25, .5, .75, .95, 0.9725 ,.99))
quantile(unlist(IQR_BOOTS_INTERVALS_crash[,"First break"]), c(.01, 0.025, .05, .1, .25, .5, .75, .95, 0.9725 ,.99))
quantile(unlist(IQR_BOOTS_INTERVALS_crash[,"Second break"]), c(.01, 0.025, .05, .1, .25, .5, .75, .95, 0.9725 ,.99))
```


```{r}
IQR_BOOTS_INTERVALS_crash <- as.data.frame(IQR_BOOTS_INTERVALS_crash)
IQR_BOOTS_INTERVALS_crash <- IQR_BOOTS_INTERVALS_crash %>%
    mutate(`t-stat` = as.numeric(`t-stat`), 
           `First break` = as.numeric(`First break`),
           `Second break` = as.numeric(`Second break`))
IQR_BOOTS_INTERVALS_crash %>%
    ggplot(aes(x = `t-stat`)) +
    geom_histogram(aes(y = ..density..),
                 colour = 1, fill = "firebrick3",
                 bins =30,
